...
| 販売管理 | CATAWIKI / EBAY / その他、在庫状態、販売ステータス、価格、手数料、粗利を管理 | 完了 | 実データで項目過不足確認 |
| 経営ダッシュボード | 月別売上、粗利、利益率、担当者別粗利、販売先別粗利、長期在庫を含む未処理タスクを表示 | 完了 | 実データで見やすさ確認 |
...
- 販売管理で未登録在庫の一括作成、在庫状態の一括変更ができる
- 仕入一覧と販売管理で在庫あり、出品中在庫、在庫なし、仕入30日超、出品30日超を確認できる
...
4. 未登録在庫の一括作成と在庫状態の一括変更を確認する
5. 経営ダッシュボードの月別売上・粗利・長期在庫タスクを確認する
6. Googleスプレッドシートへ送信し、各タブを確認する
7. 月次税理士提出ZIPを作成し、税理士確認に回す
8. [internal-test-plan.md](internal-test-plan.md) に沿って社内テストを実施する
9. 必要になった時点でstaff / tax_accountantの実アカウントを追加する
10. Version1 Release判定を行う
